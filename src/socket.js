...
export var join_game = (socket, game, player) => {
  return socket.channel("game:" + game, {
    screen_name: player
...
export var place_island = (channel, player, island, row, col) => {
  return channel.push("place_island", {player, island, row, col}).receive("ok", response(() => {
    return console.log("Island placed.", response);
  })).receive("error", response(() => {
    return console.log("Could not place island.", response);
  }));
};

export var set_islands = (channel, player) => {
  return channel.push("set_islands", player).receive("ok", response(() => {
    console.log("Board:");
    return console.dir(response.board);
  })).receive("error", response(() => {
    return console.log(`Failed to set ${player}'s island.`);
  }));
};

// game_channel.on("islands_set", response => {console.log("Player has set all islands", response)})
export var guess_coordinate = (channel, player, row, col) => {
  return channel.push("guess_coordinate", {player, row, col}).receive("error", response(() => {
    return console.log(`${player} could not guess.`, response);
  }));
};

// game_channel.on("guessed_coordinate", response => {console.log("Player made guess:", response.result)})
export var exit = (channel) => {
  return channel.exit().receive("ok", response(() => {
    return console.log("Exited channel", response);
  })).receive("error", response(() => {
    return console.log("Failed to exit channel", response);
  }));
};
